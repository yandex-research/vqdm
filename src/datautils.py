...
def get_diffusion_prompts(name, nsamples=128, seed=0, eval_mode=False):
    set_seed(seed)
    if name == "pickscore":
        if eval_mode:
            data = list(pd.read_csv("dataset/pickscore_filtered_prompts_val.csv")["caption"])
        else:
            data = list(pd.read_csv("dataset/pickscore_filtered_prompts_train.csv")["caption"])
    else:
        if name[:-4] == ".csv":
            data = list(pd.read_csv(name)["caption"])
        else:
            with open(name, "r") as f:
                data = [s.strip() for s in f]
    return list(np.random.choice(data, size=nsamples, replace=False))
