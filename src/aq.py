...
from src.modelutils import is_signed
from src.utils import ellipsis, maybe_script

logger = logging.getLogger("aq")
logging.basicConfig(stream=sys.stdout, level=logging.INFO, format="[%(asctime)s][aq] %(message)s")
...
class QuantizedLinear(nn.Module):
    ...
    def _forward(self, input: torch.Tensor):
        # TODO recheck that this is intended
        return F.linear(input, self.quantized_weight().to(input.dtype), self.bias)
...
class QuantizedConv2D(nn.Module):
    ...
    @classmethod
    def from_conv(cls, quantized_weight: QuantizedWeight, conv2d: nn.Conv2d, group_channels: bool):
        assert isinstance(conv2d, nn.Conv2d)
        assert conv2d.groups == 1, "conv groups are not implemented yet (but can be implemented trivially)"
        return cls(
            quantized_weight,
            bias=conv2d.bias,
    ...
    def _forward(self, input: torch.Tensor):
        if self.group_channels:
            conv_weight = (
                ...
                .reshape(self.out_channels, *self.kernel_size, self.in_channels)
                .permute(0, 3, 1, 2)
            )
        else:
            conv_weight = self.quantized_weight().reshape(self.out_channels, self.in_channels, *self.kernel_size)
        return F.conv2d(input, conv_weight, bias=self.bias, stride=self.stride, padding=self.padding)
...
class QuantizedWeight(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        groupwise_codebooks: bool = False,
        input_channel_scales: bool = True,
        ...
        **init_kwargs,
        ...
        with torch.no_grad():
            ...
            if input_channel_scales:
                self.input_scales = nn.Parameter(
                    weight_for_init.square().sum(dim=0).sqrt() + self.EPS, requires_grad=True
                )  # [in_features]
                weight_for_init /= self.input_scales
        ...
        codes, codebooks = init_aq_kmeans(
            ...
            groupwise_codebooks=groupwise_codebooks,
            **init_kwargs,
        )
        assert codebooks.ndim == 5
        assert codebooks.shape[0] in (1, self.in_features // self.in_group_size)

        self.codebooks = nn.Parameter(codebooks, requires_grad=True)
        # ^-- [num_in_groups or 1, num_codebooks, codebook_size, out_group_size, in_group_size]
    ...
    def get_codes(self) -> torch.Tensor:
        ...
        if torch.iinfo(self.codes.dtype).bits < 32:
            return self.codes.to(torch.int32)  # cast to int32 to allow indexing if codes are int16 or uint8
        return self.codes

    def set_codes(self, new_codes: torch.Tensor, selection: Union[slice, ellipsis, torch.Tensor] = ...):
        """Get a writable view to codes, regardless of how codes are stored"""
        self.codes[selection] = new_codes.to(dtype=self.codes.dtype, device=self.codes.device)
    ...
    def forward(self, selection: Union[slice, ellipsis, torch.Tensor] = ...):
        ...
        if self.input_scales is not None:
            weight = weight * self.input_scales
        return weight
    ...
    def beam_search_update_codes_(
        ...
        **kwargs,
        ...
        """
        if self.input_scales is not None:
            XTX = XTX.mul(self.input_scales[None, :]).mul_(self.input_scales[:, None])  # todo addmm?
            reference_weight = reference_weight.div(self.input_scales[None, :])
        self.set_codes(
    ...
    def estimate_nbits_per_parameter(self) -> float:
        ...
        if self.codebook_value_nbits < 16:
            codebooks_store += 2**self.codebook_value_nbits * self.codebook_value_num_groups * group_size * 16
    ...
    *,
    ...
    if verbose:
        ...
        if sparsity_regularizer:
            ...
            sparse_str = f" sparse {sparsity * 100:.1f}% "

        elapsed = time.perf_counter() - start_time
        logger.info(
            " " * 4 + f"optimized codes ({num_in_groups=}, {num_codebooks=}, iters={num_in_groups * num_codebooks}):"
            f"  {best_loss=:.10f} {sparse_str} t={elapsed:.3f}s."
        )
    return beam_codes[0]
...
def _beam_search_select_best(
    ...
    :param beam_codes: a tensor with best weight codes, shape: [beam_size, num_out_groups, num_in_groups, num_codebooks]
    :param beam_weights: a tensor with de-quantized beam_codes, shape: [beam_size, out_features, in_features]
    :param codebooks: a tensor with look-up tables of codes, shape: [1 or num_in_groups, num_codebooks, codebook_size, out_group_size, in_group_size]
    :param scales: weight will be multiplied by this factor, [num_out_groups, num_in_groups, 1, 1]
...
def init_aq_kmeans(
    ...
    **kwargs,
    ...
    if groupwise_codebooks and num_out_groups != out_features:
    ...
    if max_points_per_centroid is not None:
        logger.info("Clustering:", max_points_per_centroid * codebook_size, "points from", weight_residue.shape[0])

    for _ in trange(num_codebooks, desc="initializing with kmeans") if verbose else range(num_codebooks):
        if use_faiss:
            assert not groupwise_codebooks
            ...
            if not groupwise_codebooks:
                ...
                codebook_i = []
                codes_i = []
                reconstructed_weight_i = []
                assert chosen_ids is None
                for in_group_index in range(num_in_groups):
                    codebook_ij, codes_ij, reconstructed_weight_ij = fit_kmeans(
                        weight_residue.reshape(num_out_groups, num_in_groups, -1)[:, in_group_index],
                        k=codebook_size,
                        max_iter=max_iter,
                        devices=devices,
                        **kwargs,
                    )
                    codebook_i.append(codebook_ij[None])
                    codes_i.append(codes_ij.reshape(num_out_groups, 1))
                    reconstructed_weight_i.append(reconstructed_weight_ij.reshape(out_features, -1))

                codebook_i = torch.cat(codebook_i, dim=0)
                codes_i = torch.cat(codes_i, dim=1)
                reconstructed_weight_i = (
                    torch.cat(reconstructed_weight_i, dim=1)
        ...
        codebook_i = codebook_i.reshape(-1, 1, codebook_size, out_group_size, in_group_size)
        weight_residue -= reconstructed_weight_i
        codes.append(codes_i)
        codebooks.append(codebook_i)
        del reconstructed_weight_i
    codebooks = torch.cat(codebooks, dim=1)
